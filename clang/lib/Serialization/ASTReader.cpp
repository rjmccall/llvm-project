...
QualType ASTReader::readTypeRecord(unsigned Index) {
  ...
  Expected<unsigned> abbrevCode = DeclsCursor.ReadCode();
  if (!abbrevCode) {
    Error(abbrevCode.takeError());
    return QualType();
  }

  ASTRecordReader reader(*this, *Loc.F);
  Expected<unsigned> recordCode =
    reader.readRecord(DeclsCursor, abbrevCode.get());
  if (!recordCode) {
    Error(recordCode.takeError());
    return QualType();
  }
  TypeCode code = TypeCode(recordCode.get());

  if (code == TYPE_EXT_QUAL) {
    QualType baseType = reader.readQualType();
    Qualifiers quals = reader.readQualifiers();
    return Context.getQualifiedType(baseType, quals);
  }

  auto maybeClass = getTypeClassForCode(code);
  if (!maybeClass) {
    Error("Unexpected code for type");
    return QualType();
  }

  serialization::AbstractTypeReader<ASTRecordReader> typeReader(reader);
...
void TypeLocReader::VisitBuiltinTypeLoc(BuiltinTypeLoc TL) {
  ...
  if (TL.needsExtraLocalData()) {
    TL.setWrittenTypeSpec(static_cast<DeclSpec::TST>(Reader.readUInt64()));
    TL.setWrittenSignSpec(static_cast<DeclSpec::TSS>(Reader.readUInt64()));
    TL.setWrittenWidthSpec(static_cast<DeclSpec::TSW>(Reader.readUInt64()));
    TL.setModeAttr(Reader.readUInt64());
  }
}
...
ASTRecordReader::readASTTemplateArgumentListInfo() {
  ...
  unsigned NumArgsAsWritten = readUInt32();
...
void ASTRecordReader::readDeclarationNameLoc(DeclarationNameLoc &DNLoc,
                                             DeclarationName Name) {
...
ASTRecordReader::readTemplateParameterList() {
  ...
  SmallVector<NamedDecl *, 16> ParamsBuffer;
  auto Params = readArray(ParamsBuffer);
...
void ASTRecordReader::readTemplateArgumentList(
                        ...
                        bool Canonicalize) {
  unsigned NumTemplateArgs = readUInt32();
...
ASTRecordReader::readCXXCtorInitializers() {
  ...
  for (unsigned i = 0; i != NumInitializers; ++i) {
    ...
    if (/*IsWritten*/readBool()) {
      unsigned SourceOrder = readUInt32();
...
ASTRecordReader::readNestedNameSpecifierLoc() {
  ASTContext &Context = getContext();
  unsigned N = readUInt32();
  NestedNameSpecifierLocBuilder Builder;
  for (unsigned I = 0; I != N; ++I) {
    NestedNameSpecifier::SpecifierKind Kind = readNestedNameSpecifierKind();
    switch (Kind) {
...
readAPFloatSemantics(ASTRecordReader &reader) {
  return llvm::APFloatBase::EnumToSemantics(
    static_cast<llvm::APFloatBase::Semantics>(reader.readUInt32()));
}

APValue ASTRecordReader::readAPValue() {
  unsigned Kind = readUInt32();
  switch (Kind) {
  case APValue::None:
    return APValue();
